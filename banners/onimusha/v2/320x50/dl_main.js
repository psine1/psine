...
function ctaReveal() {
  ...
  var revealDuration = 0.78;

  gsap.set("#cta", {
    clipPath: "inset(0 100% 0 0)",
    webkitClipPath: "inset(0 100% 0 0)"
  });
  gsap.set("#light", { x: 140, y: 6, scale: 0.9, autoAlpha: 0 });

  tl
    .to("#light", { autoAlpha: 1, duration: 0.1, ease: "power1.out" }, 0)
    .to("#light", { x: 400, duration: revealDuration, ease: "power3.out" }, revealStart)
    .to("#cta", { clipPath: "inset(0 0% 0 0)", webkitClipPath: "inset(0 0% 0 0)", duration: revealDuration, ease: "power3.out" }, revealStart)
    .to("#light", { autoAlpha: 0, duration: 0.2, ease: "power1.out" }, 0.68);
...
function fr1() {
  ...
  gsap.set("[id*='txt-1-']", { autoAlpha: 0, y: 0, scale: 1.4, transformOrigin: "20% 50%" }, 0)
  gsap.set("[id*='txt-2-']", { autoAlpha: 0, y: 0, scale: 1.4, transformOrigin: "50% 50%" }, 0)
  gsap.set("[id*='txt-3-']", { autoAlpha: 0, y: 0, scale: 1.4, transformOrigin: "50% 50%" }, 0)
  gsap.set("#fr1, #fr2", { autoAlpha: 1 }, 0)
  ...
  tl_
    ...
    .to("#brushes", { clipPath: "circle(75% at 50% 50%)", webkitClipPath: "circle(75% at 50% 50%)", duration: 1.2, ease: "power3.out" }, "<+0.1")    
    ...
    .set("#fr1", { autoAlpha: 0 }, ">")
    .from("#wrapCharacter", { duration: 10, scale: 1.2, x: 20, ease: "power2.out" }, "<-1")
    .from("#arm, #swordShine", { duration: 10, rotation: 20, ease: "power2.out" }, "<")
    
    .from("#swordShine img", { duration: 1, autoAlpha: 0, yoyo: true, repeat: -1, transformOrigin: "80% 50%", ease: "none" }, "<")

    .from("#bg2", { duration: 10, scale: 1.2, x: -20, ease: "power2.out" }, "<-1")
    .from("#logo", { duration: 0.35, scale: 1.5, autoAlpha: 0, ease: "power2.out" }, "<+1.65")
    .add(ctaReveal(), "<+0.35")

  // .from('#bg', 1.2, {y: 250, x: 50, rotation: -10, transformOrigin: "50% 50%", ease: "steps(8)" }, "<")
...
(function (window, document) {
  ...
  function start() {
    ...
    readyPromise = loadImage(IMAGE_SOURCE)
      .then(startWebGL)
  ...
  function startWebGL(image) {
    ...
    var copyProgram = createProgram(gl, vertexSource, copyFragmentSource);
    var feedbackProgram = createProgram(gl, vertexSource, feedbackFragmentSource);
    var displayProgram = createProgram(gl, vertexSource, displayFragmentSource);
    ...
    function draw(now) {
      var amount = clamp(state.amount, 0, 1);
      ...
      if (delta > 0 && amount > 0) {
        ...
        gl.uniform1f(blurLocation, clamp(settings.blur, 0, 1));
...
(function (window, document) {
  ...
  if (typeof settings.frontStrength !== "number") settings.frontStrength = 8.2;
  if (typeof settings.backStrength !== "number") settings.backStrength = 6.1;
  ...
  function startWebGL(image) {
    ...
    var fragmentSource = [
      ...
      "  vec2 scalpSpace = vec2(fromAnchor.x / 0.14, fromAnchor.y / 0.12);",
      "  float scalpDistance = length(scalpSpace);",
    ...
    function draw() {
      ...
      gl.uniform1f(timeLocation, Math.max(0, state.time) * clamp(settings.speed, 0.1, 3));
      gl.uniform1f(amountLocation, clamp(state.amount, 0, 1));
      gl.uniform1f(strengthLocation, clamp(settings.strength, 0, 1.5));
      gl.uniform1f(frontStrengthLocation, clamp(settings.frontStrength, 0.5, 2.4));
      gl.uniform1f(backStrengthLocation, clamp(settings.backStrength, 0.5, 2.4));
      gl.uniform1f(topStrengthLocation, clamp(settings.topStrength, 0.5, 2.4));
      gl.uniform1f(edgeStrengthLocation, clamp(settings.edgeStrength, 0.5, 2.4));
      gl.drawArrays(gl.TRIANGLES, 0, 6);
      animationFrame = requestAnimationFrame(draw);
    }
...
(function (window, document) {
  ...
  if (typeof settings.strength !== "number") settings.strength = 3;
  if (typeof settings.speed !== "number") settings.speed = 10;
