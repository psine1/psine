document.addEventListener('DOMContentLoaded', () => {
	...
	const conceptButtons = Array.from(document.querySelectorAll('[data-bn-concept]'));
	const formatCampaignRoot = formatStage?.dataset.bnCampaignRoot?.replace(/\/+$/, '') || '';
	const formatCampaignName = formatStage?.dataset.bnCampaignName || '';
	let activeFormatConcept = conceptButtons.find((button) => button.classList.contains('is-active'))?.dataset.bnConcept || '';

	const getFormatSource = (button) => {
		if (!button) return '';
		if (!formatCampaignRoot || !activeFormatConcept) return button.dataset.src || '';
		return `${formatCampaignRoot}/${activeFormatConcept}/${button.dataset.bnFormatSize}/index.html`;
	};

	const getFormatTitle = (button) => {
		if (!button || !formatCampaignName || !activeFormatConcept) return button?.dataset.title || '';
		const conceptLabel = conceptButtons.find((item) => item.dataset.bnConcept === activeFormatConcept)?.textContent.trim() || activeFormatConcept;
		const formatLabel = button.dataset.bnFormatSize.replace('x', ' by ');
		return `${formatCampaignName} ${conceptLabel} ${formatLabel} banner`;
	};

	const loadFormat = (button) => {
		if (!formatFrame || !button) return;
		const source = getFormatSource(button);
		if (source) setFrameSource(formatFrame, source);
		formatFrame.title = getFormatTitle(button);
	};
	...
	conceptButtons.forEach((button) => {
		button.addEventListener('click', () => {
			activeFormatConcept = button.dataset.bnConcept;
			...
			const activeFormat = formatButtons.find((item) => item.classList.contains('is-active')) || formatButtons[0];
			loadFormat(activeFormat);
		});
	});
